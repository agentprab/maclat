...
import {
  insertPoster, getPoster,
  insertAgent, getAgent, updateAgentStatus, destroyAgentWallet,
  insertJob, getJob, getAvailableJobs, getJobsByPoster, claimJob, updateJobStatus,
  insertEscrow, getJobEscrow, releaseEscrow,
  insertUpdate, getJobUpdates,
  insertDeliverable, getJobDeliverables,
  insertInstruction, getPendingInstructions, markInstructionDelivered,
  deleteJob,
} from './db.js';
import { generateTempWallet } from './escrow.js';
import { broadcastJobEvent, subscribeToJob } from './sse.js';

export function createRoutes(app: Hono, db: Database.Database): void {
  ...
  app.post('/register', async (c) => {
    const { name, wallet_address } = await c.req.json<{ name: string; wallet_address: string }>();
    if (!name || !wallet_address) return c.json({ error: 'name and wallet_address required' }, 400);
  ...
  app.post('/jobs', async (c) => {
    const { poster_id, title, description, budget_usdc } = await c.req.json<{ poster_id: string; title: string; description: string; budget_usdc: number }>();
    if (!poster_id || !title || !budget_usdc) return c.json({ error: 'poster_id, title, budget_usdc required' }, 400);
  ...
  app.get('/jobs', (c) => {
    ...
    if (posterId) return c.json(getJobsByPoster(db, posterId));
    return c.json(getAvailableJobs(db));
  });
  ...
  app.post('/jobs/:id/claim', async (c) => {
    ...
    const job = getJob(db, id);
    if (!job) return c.json({ error: 'job not found' }, 404);
    if (job.status !== 'open') return c.json({ error: `job status is ${job.status}, not open` }, 400);
  ...
  app.post('/jobs/:id/updates', async (c) => {
    const id = c.req.param('id');
    const { agent_id, type, content } = await c.req.json<{ agent_id: string; type: string; content: string }>();
  ...
  app.post('/jobs/:id/deliver', async (c) => {
    const id = c.req.param('id');
    const { agent_id, files, summary } = await c.req.json<{ agent_id: string; files: Array<{ path: string; content: string }>; summary: string }>();
  ...
  app.get('/jobs/:id/files/:path{.+}', (c) => {
    ...
    const files = JSON.parse(d.files || '[]') as Array<{ path: string; content: string }>;
  ...
  app.post('/jobs/:id/approve', async (c) => {
    ...
    const job = getJob(db, id);
    if (!job) return c.json({ error: 'job not found' }, 404);
    if (!job.agent_id) return c.json({ error: 'no agent assigned' }, 400);
