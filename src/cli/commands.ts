...
import type { ExecutorType } from '../shared/types.js';
...
// --- Use (set executor) ---
const VALID_EXECUTORS: ExecutorType[] = ['claude-code', 'anthropic', 'openrouter', 'codex'];

export function useExecutor(args: string[]): void {
  const executor = args[0] as ExecutorType | undefined;
  ...
  if (config.api_key) console.log(`  API Key:  ***${config.api_key.slice(-4)}`);
