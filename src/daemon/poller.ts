...
import type { JobExecutor, OnUpdate, Interactivity } from './executor.js';
import { POLL_INTERVAL_MS, JOBS_DIR } from '../shared/config.js';
import { join } from 'path';
import {
  bold, dim, gray, green, yellow, red, cyan,
  toolPill, fileWrite as fmtFileWrite, agentText, jobFound, jobDone, jobError, costLine,
} from './format.js';
      ...
      const interactivity: Interactivity = {
        getInstructions: async () => {
          try {
            ...
            if (!res.ok) return [];
            return await res.json() as Array<{ id: string; content: string }>;
          } catch { return []; }
        },
